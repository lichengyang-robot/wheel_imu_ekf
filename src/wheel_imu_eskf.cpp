...
void encoderCallback(const irp_sen_msgs::encoder::ConstPtr& msg) {
    ...
    // 计算位姿更新（使用差分法）
    double delta_theta = angular_velocity * dt + 0.00001 ;
...
void publishPath(const sensor_msgs::Imu::ConstPtr &imu_msg) {
    ...
    pose_msg.pose.orientation.x = imu_msg->orientation.x;
    pose_msg.pose.orientation.y = imu_msg->orientation.y;
    pose_msg.pose.orientation.z = imu_msg->orientation.z;
    pose_msg.pose.orientation.w = imu_msg->orientation.w;
...
void predictState(const sensor_msgs::Imu::ConstPtr &imu_msg, double dt) {
    ...
    F(0, 2) = dt; 
    F(1, 3) = dt; 
    F(2, 4) = -old_state.velocity(1) * sin(state.theta) * dt; 
    F(3, 4) = old_state.velocity(0) * cos(state.theta) * dt; 

    // 4. 计算控制输入矩阵 B
    Eigen::MatrixXd B = Eigen::MatrixXd::Zero(5, 2);
    B(0, 0) = 0.5 * dt * dt * cos(state.theta); 
    B(1, 0) = 0.5 * dt * dt * sin(state.theta); 
    B(2, 0) = dt * cos(state.theta);  
    B(3, 0) = dt * sin(state.theta);  
    B(4, 1) = dt; 

    // 5. 计算控制输入向量 u
    Eigen::Vector2d u;
    u << acc_world(0), delta_angular;

    // 6. 计算状态
    state.position += state.velocity * dt + 0.5 * acc_world.head<2>() * dt * dt;
    state.velocity += acc_world.head<2>() * dt;
...
void updateObservation_eskf(const nav_msgs::Odometry::ConstPtr &wheel_msg) {
    ...
    double wheel_delta_angular = wheel_msg->twist.twist.angular.z * delta_t; // 从轮速计得到的角速度
...
void syncData() {
    if(imu_queue.size() < 50 && wheel_odom_queue.size()< 50 ){
        ...
        if (!initialized && wheel_msg->twist.twist.linear.x != 0) {
            ...
            double imu_delta_angular = 0.0; // 从IMU估计的角速度
            double wheel_delta_angular = 0.0; // 从轮速计得到的角速度
            // 使用加权平均来融合IMU和轮速计的角速度估计
            double alpha = 0.5;  // 融合权重
            double fused_delta_angular = alpha * imu_delta_angular + (1 - alpha) * wheel_delta_angular;
            state.theta = fused_delta_angular;
            ...
            // 初始协方差矩阵设定较大值，以便逐步调整
            state.P = Eigen::MatrixXd::Identity(5, 5) * 1000;
            state.R.block<2, 2>(0, 0) = Eigen::MatrixXd::Identity(2, 2) * 10000; // 高协方差，允许不确定性
            state.R.block<2, 2>(2, 2) = Eigen::MatrixXd::Identity(2, 2) * 100;
            state.R.block<1, 1>(4, 4) = Eigen::MatrixXd::Identity(1, 1) * 10000;
...
int main(int argc, char **argv) {
    ...
    state.theta = 0.0;
    state.P = Eigen::MatrixXd::Identity(5, 5) * 3.0;
